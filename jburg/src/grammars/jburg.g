...
guard_specifications!
    ...
    :
    (GUARD procall:procedure_call { result.addChild(#procall); } )*
    {
        #guard_specifications = result;
    }
    ;
...
rule!:
        ...
        guards: guard_specifications
        action:reduction_action    
    {
        #rule = #([PATTERN_RULE], nonTerminalRuleId, pattern, cost, guards, action);
    }
...
reduction_declaration!:
    nonTerminalRuleId:IDENTIFIER EQUALS  PATTERN name:IDENTIFIER cost:cost_specification guards:guard_specifications r:reduction_action
    {
        #reduction_declaration = #([REDUCTION_DECLARATION], nonTerminalRuleId, name, cost, guards, r);
    }
    ;
