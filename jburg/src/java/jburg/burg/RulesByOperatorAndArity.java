...
class RulesByOperatorAndArity
{
    ...
    {
        assert !analyzed();
        annotationSemantics = new TreeMap<String, List<AnnotationSemantics>>();
    ...
    private List<AnnotationSemantics> partition(Collection<JBurgRule> unsorted_rules)
    {
        ...
        //  Any rules with guard functions must be tried
        //  in the order they were specified in the specification.
        for ( JBurgRule rule: unsorted_rules ) {
            if (rule.hasGuardFunctions()) {
                rules.addToSet(rule.patternMatcher.getNominalArity(), rule);
            }
        }
