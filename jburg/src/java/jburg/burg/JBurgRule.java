...
public class JBurgRule implements JBurgProduction
{
    ...
    /**
     * Guard functions for the rule;
     * the rule is selected if all guards return true.
     */
    final Vector<AST> guardFunctions = new Vector<AST>();
    ...
    public JBurgRule(AST n, JBurgGenerator generator)
    {
        ...
        if ( m_AST.getType() == PATTERN_RULE ) {
            ...
            try {
                ...
                generator.getLogger().exception("Building pattern recognizer for " + m_AST.toStringTree(), ex);
            }

            AST guardRoot = getASTByType(m_AST, GUARD);
            for (AST guard = guardRoot.getFirstChild(); guard != null; guard = guard.getNextSibling()) {
                this.guardFunctions.add(guard);
            }
    ...
    {
        if (this.hasGuardFunctions())
            // Guards are eagerly evaluated at annotation selection.
            return false;
        else if ( this.isTerminalPattern() ) 
            return false;
            ...
            return emitter.getTemplate(
                ...
                "expr", emitter.getTemplate(
                    ...
                    "value",    generator.getCodeEmitter().genPopFromStack(
                        ...
                        generator.getReturnType(this.getGoalState())
                    )
                )
            ).toString();
	...
	{   
        ...
        {
            if ( generator.patternMatcherDumpFile != null )
            {
                //  Dump the BURM's debugging info.
                patternBURM.dump(new PrintWriter(generator.patternMatcherDumpFile));
            }
    ...
    public boolean hasGuardFunctions()
    {
        return this.guardFunctions.size() > 0;
    }
}
