...
public class StaticLinkTest {
    ...
    public void twoLevelTest() {
        String tigerCode = "let var a:int := 1 function d():int = let function e():int = a in  e() end in d() end";
        InputStream inputStream = new ByteArrayInputStream(tigerCode.getBytes(Charset.forName("UTF-8")));
        Main m = new Main("chap5", inputStream);
        m.compile();
        assertFalse(m.hasErrors());
    ...
    public void formal_inframe_acesss_local_inframe_access() {
        ...
        String tigerCode = "let function d():int = let var a:int := 99 in a end in d() end";
        InputStream inputStream = new ByteArrayInputStream(tigerCode.getBytes(Charset.forName("UTF-8")));
        Main m = new Main("chap5", inputStream);
        m.compile();
        assertFalse(m.hasErrors());
